...
function getSourcesQueryOptions(pluginKey: string) {
  return {
    queryFn: () =>
      request(OpenAPI, {
        method: "GET",
...
export const Route = createFileRoute("/_layout/plugin/$pluginKey")({
...
function SourcesTableContent() {
  ...
  const { data: sources } = useSuspenseQuery(getSourcesQueryOptions(pluginKey))
