...
import {
  type CanonicalTitleOutput,
  CanonicalTitlesService,
  TitlesService,
} from "@/client"
...
interface CanonicalTitleFieldProps {
  titleId: string
...
// TODO: Validate
function CanonicalTitleName({ title }: { title: CanonicalTitleOutput }) {
  return (
...
// TODO: Validate
/**
 * Which canonical titles this row stands for, and the choosing of another.
 *
 ...
 * do.
 */
export function CanonicalTitleField({
  titleId,
  canonicalTitleIds,
  enabled,
}: CanonicalTitleFieldProps) {
  ...
  const linkedQueries = useQueries({
    queries: canonicalTitleIds.map((canonicalTitleId) => ({
      queryKey: ["canonical-title", canonicalTitleId],
      queryFn: () =>
        CanonicalTitlesService.getCanonicalTitleById({ canonicalTitleId }),
      enabled,
    })),
  })
  ...
  const { data: results, isFetching } = useQuery({
    queryKey: ["canonical-titles", "search", search],
    queryFn: () =>
      CanonicalTitlesService.getCanonicalTitles({
  ...
  const linkMutation = useMutation({
    mutationFn: (chosenId: string) =>
      TitlesService.adminLinkTitleToCanonical({
        titleId,
        canonicalTitleId: chosenId,
      }),
    onSuccess: () => {
      showSuccessToast("Title linked to canonical title")
      setSearch("")
      rereadTitle()
    },
  ...
  const unlinkMutation = useMutation({
    mutationFn: (droppedId: string) =>
      TitlesService.adminUnlinkTitleFromCanonical({
        titleId,
        canonicalTitleId: droppedId,
      }),
    onSuccess: () => {
      showSuccessToast("Title unlinked from canonical title")
      rereadTitle()
    },
    onError: (error: unknown) =>
      handleError.call(
  ...
  const linkedIds = new Set(canonicalTitleIds)
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor="canonical-title-search">Canonical Titles</Label>
      {canonicalTitleIds.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Linked to no canonical title.
        </p>
      ) : (
        <div className="rounded-lg border">
          {canonicalTitleIds.map((canonicalTitleId) => {
            const title = linked.find((each) => each.id === canonicalTitleId)
            return (
              <div
                key={canonicalTitleId}
                className="flex items-center gap-3 border-b px-3 py-2 text-sm last:border-b-0"
              >
                {title ? (
                  <CanonicalTitleName title={title} />
                ) : (
                ...
                <Button
                  ...
                  size="icon"
                  className="shrink-0"
                  title="Unlink from this canonical title"
                  disabled={unlinkMutation.isPending}
                  onClick={() => unlinkMutation.mutate(canonicalTitleId)}
                >
                  <X className="h-4 w-4" />
                </Button>
              </div>
      ...
      <Input
        ...
        placeholder="Search canonical titles by name"
      />
        ...
        <div className="max-h-64 overflow-y-auto rounded-lg border">
          {offered.length === 0 ? (
            <p className="p-3 text-sm text-muted-foreground">
              {isFetching
                ? "Searching…"
                : "No canonical title under that name."}
            </p>
          ) : (
            offered.map((title) => (
              <div
