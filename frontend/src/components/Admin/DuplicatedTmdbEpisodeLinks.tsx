...
interface DuplicatedTmdbEpisodeLinksProps {
...
// TODO: Validate
export function DuplicatedTmdbEpisodeLinks({
  episodes,
}: DuplicatedTmdbEpisodeLinksProps) {
  return (
    <div className="flex min-w-64 flex-col gap-1 py-1">
      {episodes.map((record) => (
        <div key={record.episode.id} className="flex items-center gap-2">
          ...
          {record.episode.tmdb_episode_validated_at ? (
