...
import { type SnapshotEpisodesOutput, WatchesService } from "@/client"
...
type SnapshotEpisode = BaseEpisodeWithDetails & {
  watch_date?: string | null
  verified?: boolean | null
  episode_watch_id?: string | null
}

interface SnapshotEpisodeCardProps {
...
function updateEpisodeInData(
  oldData: SnapshotEpisodesOutput | undefined,
  episodeId: string,
  patch: Partial<SnapshotEpisode>,
): SnapshotEpisodesOutput | undefined {
...
export function SnapshotEpisodeCard({
  episode,
  snapshotId,
  onHide,
  editOrder,
  ...
  const queryKeys = snapshotId
    ? [
        ["snapshot-episodes", snapshotId] as const,
        ["snapshot-episodes-preview", snapshotId] as const,
      ]
    : []

  const patchCaches = (episodeId: string, patch: Partial<SnapshotEpisode>) => {
    for (const key of queryKeys) {
      queryClient.setQueriesData<SnapshotEpisodesOutput>(
        { queryKey: key },
  ...
  const onCardClick = () => {
    if (snapshotId) markWatchedMutation.mutate(episode.id)
    if (episode.url) {
