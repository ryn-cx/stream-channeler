...
class SearchMixin(HelperMixin, register=False):
    ...
    def search(self, query: str, cursor: str | None = None) -> PluginSearchResults:
        ...
        results = [
            ...
            if result["metrics_info"]["target_type"]
            in (SERIES_MEDIA_TYPE, MOVIE_MEDIA_TYPE)
        ]
    ...
    def _search_result(self, result: dict[str, Any]) -> PluginSearchResult:
        ...
        premiere_date = result["entity_metadata"].get("premiere_date")
        image_path = result["visuals"]["artwork"]["horizontal"]["image"]["path"]
        return PluginSearchResult(
            title=metrics["target_name"],
            url=self._show_url(title_key, media_type),
            year=tz_datetime.fromisoformat(premiere_date).year
            if premiere_date
            else None,
