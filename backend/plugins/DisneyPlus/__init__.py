...
class DisneyPlus(HelperMixin, URLHandlerPlugin[DisneyPlusURLHandler], register=True):
    """Disney+ plugin."""
    ...
    def _upsert_tv_seasons(
        ...
        *,
        ...
        for sort_order, season_entry in enumerate(self._seasons(show_key)):
            season_id = str(season_entry.id)
            season_key = self._season_key(show_key, season_id)
            ...
            self._upsert_tv_episodes(season, show_key, season_id, force=force)
