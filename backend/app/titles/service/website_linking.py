...
# TODO: Validate
def link_title_to_websites(session: Session, title: Title) -> None:
    ...
    for provider in TMDB(session, title.source.plugin).streaming_providers(title):
        ...
        if media_plugin is None:
            _upsert_unmatched_source(session, title.id, provider.provider_name, None)
            continue

        if _external_link_exists(
            session=session,
...
# TODO: Validate
def _external_link_exists(  # noqa: PLR0913 - The session joins what was a method's arguments.
    session: Session,
...
def _import_external_plugin(  # noqa: PLR0913 - The session joins what was a method's arguments.
    ...
    if not plugin_class.implements("search_for_title_url"):
        return False
