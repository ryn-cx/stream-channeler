...
def upgrade():
    ...
    op.create_index(
        "ChannelFavorite-channel_id-index",
        "channelfavorite",
        ["channel_id"],
        unique=False,
    )
    op.create_table(
        "channelorderfavorite",
        sa.Column("id", sa.Uuid(), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("modified_at", sa.DateTime(timezone=True), nullable=False),
        sa.Column("user_id", sa.Uuid(), nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["channel_order_id"], ["channelorder.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["user_id"], ["user.id"], ondelete="CASCADE"),
    ...
    op.create_index(
        "ChannelOrderFavorite-channel_order_id-index",
        "channelorderfavorite",
        ["channel_order_id"],
        unique=False,
    )
...
def downgrade():
    op.drop_index(
        "ChannelOrderFavorite-channel_order_id-index", table_name="channelorderfavorite"
    )
