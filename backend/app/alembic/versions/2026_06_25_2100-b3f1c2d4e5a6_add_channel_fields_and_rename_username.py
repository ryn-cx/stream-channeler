...
def upgrade():
    ...
    op.add_column('channel', sa.Column('description', sqlmodel.sql.sqltypes.AutoString(), nullable=True))
    op.add_column('channel', sa.Column('anonymous', sa.Boolean(), nullable=False, server_default=sa.false()))
    op.alter_column('channel', 'anonymous', server_default=None)
    op.add_column('channel', sa.Column('score', sa.Integer(), nullable=False, server_default='0'))
